use rumax::models::{Identity, UserAgent};
use rand::seq::SliceRandom;
use uuid::Uuid;

const APP_VERSION: &[(&str, i32)] = &[
    ("26.25.0", 6790), ("26.26.0", 6797), ("26.26.1", 6798),
    ("26.27.0", 6800), ("26.27.1", 6802), ("26.28.0", 6804),
    ("26.29.0", 6807), ("26.29.1", 6808), ("26.30.0", 6810),
    ("26.30.1", 6819), ("26.31.0", 6822),
];

const ANDROID_DEVICES: &[(&str, &str, &str, &str)] = &[
...
pub fn generate_device() -> Identity {
    let mut rng = rand::thread_rng();

    let app = APP_VERSION.choose(&mut rng).unwrap();
    let dev = ANDROID_DEVICES.choose(&mut rng).unwrap();
    let loc = LOCALE_TIMEZONES.choose(&mut rng).unwrap();

    let mt_instance = Uuid::new_v4().to_string();
    ...
    Identity {
        ...
        user_agent: UserAgent {
            ...
            app_version: app.0.to_string(),
            os_version: dev.1.to_string(),
            timezone: loc.1.to_string(),
            screen: dev.2.to_string(),
            push_device_type: Some("GCM".to_string()),
            arch: Some(dev.3.to_string()),
            locale: loc.0.to_string(),
            build_number: app.1,
            device_name: dev.0.to_string(),
            device_locale: loc.0.to_string(),
            release: None,
